...
export function MenuItem(props: { name: string; description: string, orderUrl: string, pizza_type: string, img_obj: any }) {
  ...
  return (
    <button className={styles.menuItem} onClick={onClickHandler}>
      <div className={styles.menuItem__imgArea}>
        <img className={styles.menuItem__img} src={props.img_obj} alt="cheese pizza" />
      </div>
