...
def test_finish_valid_pizza():
    ...
    # Action
    finish_pizza_response = requests.put(url=f"{config.back_end_url}/api/pizza/{new_pizza_id}/finish")

    # Assert
    assert finish_pizza_response.ok
    response_content = finish_pizza_response.json()
    assert response_content["status"] == "success"


def test_finish_invalid_pizza():
    # Action
    uuid_factory = UUIDFactory()
    new_id = uuid_factory.create_new_id()
    finish_pizza_response = requests.put(url=f"{config.back_end_url}/api/pizza/{new_id}/finish")
